...
namespace auto_aim
{
YOLOV5::YOLOV5(const std::string & config_path, bool debug): debug_(debug)
{
...
binary_threshold_ = yaml["threshold"] ? yaml["threshold"].as<double>() : 0.5;
min_confidence_   = yaml["min_confidence"] ? yaml["min_confidence"].as<double>() : 0.5;
int x = 0, y = 0, width = 0, height = 0;
x      = yaml["roi"]["x"]      ? yaml["roi"]["x"].as<int>()      : 0;
y      = yaml["roi"]["y"]      ? yaml["roi"]["y"].as<int>()      : 0;
width  = yaml["roi"]["width"]  ? yaml["roi"]["width"].as<int>()  : 1280;
height = yaml["roi"]["height"] ? yaml["roi"]["height"].as<int>() : 720;
